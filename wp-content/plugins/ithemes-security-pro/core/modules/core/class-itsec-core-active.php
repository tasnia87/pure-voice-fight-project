	...
	public function register_tools( Tools_Registry $registry ) {
		$registry->register( new class( 'set-encryption-key', ITSEC_Modules::get_config( 'core' ) ) extends Config_Tool {
			public function run( array $form = [] ): Result {
				if ( ITSEC_Lib_Encryption::is_available() && ! $form['confirm'] ) {
					return Result::error( new WP_Error(
						...
						__( 'You must check “Confirm Reset Key” to continue.', 'it-l10n-ithemes-security-pro' )
					) );
				}
				...
				try {
					...
					return Result::error( new WP_Error(
						...
						__( 'Could not generate a random encryption key.', 'it-l10n-ithemes-security-pro' )
					) );
				}
			...
			public function get_help(): string {
				...
				$help .= ' ' . __( 'Your site already has a valid encryption key. Use this tool to automatically re-encrypt all secrets with a new encryption key.', 'it-l10n-ithemes-security-pro' );
				$help .= ' ' . __( 'This may take a while. If available, try running this tool with WP CLI for better performance.', 'it-l10n-ithemes-security-pro' );
	...
	public function rotate_encrypted_user_keys( User_Key_Rotator $rotator, Result $result ) {
		...
		if ( $wpdb->last_error ) {
			$result->add_warning_message(
				sprintf( __( 'Could not fetch user metadata to update: %s', 'it-l10n-ithemes-security-pro' ), $wpdb->last_error )
			);
		...
		foreach ( $rows as $row ) {
			...
			try {
				...
				if ( $updated ) {
					...
					$result->add_warning_message(
						sprintf(
							__( 'Could not rotate \'%1$s\' for \'%2$d\': %3$s', 'it-l10n-ithemes-security-pro' ),
							$meta_key,
							$user_id,
							$wpdb->last_error ?: __( 'User meta not updated.', 'it-l10n-ithemes-security-pro' )
						)
					);
				}
			} catch ( RuntimeException $e ) {
