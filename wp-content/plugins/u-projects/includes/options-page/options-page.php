			...
			if(file_exists($this->_args['option_file'])){
				try {
					...
					foreach($sxe->tab as $tab){
						...
						$tab_titles .= '<li ' . ($tabidx == 1 ? 'class="uk-active"':'') .'><a href="#option-tab-'.$tabidx.'">'.$icon. esc_html__(''.$tab_name,'cactusthemes').'</a></li>';
						...
						foreach($tab->group as $group){
							...
							$tab_contents .= 
								'<table class="uk-table" width="100%" cellpadding="0" cellspacing="0">';
									...
									$tab_contents .= '
									<thead>
										<th>'.$icon. esc_html__(''.$group_name,'cactusthemes').'</th>
									</thead>';
									}
							...
							foreach($group->fields as $fields){
								...
								$tab_contents .=
									'<tr class="row label '.(count($fields->description) > 0?'no-border':'').'">
										<td align="left">
											<label>'
												. esc_html__(''.$fields_atts['label'],'cactusthemes').'</label>
										</td>
									</tr>
									<tr class="row">
										<td>';
											foreach($fields->option as $option){
												...
												if(isset($atts['tooltip'])){
													$props = ' data-uk-tooltip title="' . esc_html__(''.$atts['tooltip'],'cactusthemes') . '" ';
												}
												...
												if(isset($atts['label'])){
													$tab_contents .= '<label>' . esc_html__(''.$atts['label'],'cactusthemes') . ':</label>';
												}
									...
									if(count($fields->description) > 0){
										foreach($fields->description as $desc){
											$tab_contents .= '<tr class="row description"><td>'.esc_html__(''.$desc,'cactusthemes').'</td></tr>';
										}
									}
							}
					...
					echo esc_html__("Exception occurs: ",'cactusthemes') . print_r($e);
				}
			}
...
margin-left: -1px;">' .
		...
		public function get($option_name){
			if($this->_options){
				return isset($this->_options[$option_name]) ? $this->_options[$option_name] : '';
			}
		}
	}	
}
...
if(!function_exists('cop_get')){
	function cop_get($op_id, $option_name){
		if(isset($GLOBALS[$op_id]) && $GLOBALS[$op_id] && is_a($GLOBALS[$op_id], 'Options_Page')){			
