...
class CMB_Date_Timestamp_Field extends CMB_Field {
	...
	public function html() { ?>
		...
		<input <?php $this->id_attr(); ?> <?php $this->boolean_attr(); ?> <?php $this->class_attr( 'cmb_text_small cmb_datepicker' ); ?> type="text" <?php $this->name_attr(); ?>  value="<?php echo $this->value ? esc_attr( date( 'Y\/m\/d', $this->value ) ) : '' ?>" date-format="<?php echo isset($this->args['dateFormat']) ? $this->args['dateFormat'] : 'yy/mm/dd';?>" />
...
class CMB_Datetime_Timestamp_Field extends CMB_Field {
	...
	public function html() { ?>
		...
		<input <?php $this->id_attr('date'); ?> <?php $this->boolean_attr(); ?> <?php $this->class_attr( 'cmb_text_small cmb_datepicker' ); ?> type="text" <?php $this->name_attr( '[date]' ); ?>  value="<?php echo $this->value ? esc_attr( date( 'Y\/m\/d', $this->value ) ) : '' ?>" date-format="<?php echo isset($this->args['dateFormat']) ? $this->args['dateFormat'] : 'yy/mm/dd';?>" />
...
class CMB_Select extends CMB_Field {
	...
	public function enqueue_scripts() {
        ...
        if(!in_array($screen->id, array('product', 'shop_coupon', 'shop_order', 'shop_subscription', 'wc_membership_plan'))){
			wp_enqueue_script( 'cmb-select2', trailingslashit( CMB_URL ) . 'js/vendor/select2/select2.js', array( 'jquery' ) );
			wp_enqueue_script( 'cmb-field-select', trailingslashit( CMB_URL ) . 'js/field.select.js', array( 'jquery', 'cmb-select2', 'cmb-scripts' ) );
		}
	}
	...
	public function enqueue_styles() {
		...
		$screen = get_current_screen();
        if(!in_array($screen->id, array('product', 'shop_coupon', 'shop_order', 'shop_subscription', 'wc_membership_plan'))){
			wp_enqueue_style( 'cmb-select2', trailingslashit( CMB_URL ) . 'js/vendor/select2/select2.css' );
		}
	}
