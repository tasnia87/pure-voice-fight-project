...
function mo_saml_login_validate() {
	...
	if ( isset( $_REQUEST['option'] ) && 'mo_fix_iconv_cert' === $_REQUEST['option'] && is_user_logged_in() && current_user_can( 'manage_options' ) ) {
		update_option( Mo_Saml_Options_Enum_Service_Provider::IS_ENCODING_ENABLED, '' );
		wp_safe_redirect( '?option=testConfig' );
		exit;
	}
	...
	if ( ! empty( $_POST['SAMLResponse'] ) ) {
		...
		$saml_is_encoding_enabled = get_option( Mo_Saml_Options_Enum_Service_Provider::IS_ENCODING_ENABLED );
		if ( ! $valid_signature ) {
...
function mo_saml_convert_to_windows_iconv( $cert_fp_from_plugin ) {
	$encoding_enabled = get_option( Mo_Saml_Options_Enum_Service_Provider::IS_ENCODING_ENABLED );

	if ( '' === $encoding_enabled || ! Mo_SAML_Utilities::mo_saml_is_iconv_installed() ) {
		return $cert_fp_from_plugin;
	}
	return iconv( 'UTF-8', 'CP1252//IGNORE', $cert_fp_from_plugin );
}
