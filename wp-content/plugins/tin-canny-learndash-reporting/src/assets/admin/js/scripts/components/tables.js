var reportingTables = {
    ...
    getColumnDefs: function (tableType) {
        ...
        switch (tableType) {
            ...
            case 'userSingleCoursesOverviewTable':
                ...
                orderableCols = [];
                nonOrderableCols = [2, 4, 5];
                if (typeof reportingApiSetup.disablePerformanceEnhancments !== 'undefined' && '1' === reportingApiSetup.disablePerformanceEnhancments) {
                    orderableCols = [2];
                    nonOrderableCols = [4, 5];
                }
                ...
                columnDefs = [
                    ...
                    {
                        "targets": [3], "visible": true, "searchable": false, "orderable": false, render: {
                            _: 'display',
                            sort: 'timestamp'
                        }
                    },
            ...
            if ('userSingleCourseProgressSummaryTable' === tableType) {
                ...
                apiDataCall.done(function (response) {
                    ...
                    if('' !== response.data.course_certificate ){
                        certifcateLink = '<a target="_blank" href="' + response.data.course_certificate + '">View</a>';
                    }
                    ...
                    for (let row in quizzes) {
                        if (quizzes.hasOwnProperty(row)) {
                            ...
                            if('' !== quizzes[row].certificate_link){
                                certifcateLink = '<a target="_blank" href="' + quizzes[row].certificate_link + '">View</a>';
                            }
    ...
    drawUserSingleCoursesOverviewTable: function (response) {
        ...
        for (let row in response.data) {
            ...
            if ('' !== response.data[row].completed_date.display) {
                coursesCompleted++;
            } else if ('' === response.data[row].progress || 0 === response.data[row].progress) {
                coursesNotStarted++;
            } else {
                coursesInProgress++;
            }
    ...
    courseSingleTableData: function (courseID) {
        ...
        jQuery.each(userAccess, function (key, userID) {
            ...
            // User Display Name
            userDisplayName = `<span class="reporting-table-see-details">${ userDisplayName }</span>`;
